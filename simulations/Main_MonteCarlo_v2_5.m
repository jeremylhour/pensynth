...
parfor t = 1:T
    ...
    % 1. BC Penalized Synthetic Control w/ optimized lambda
    % Here lambda is set to optimize the BC MSE
    ...
    for r = rset
        ...
        while j <= length(Lambda)
            ...
            mse = (y1(:,i)-W(:,:,i)'*y0(:,i) - (mu_hat1-W(:,:,i)'*mu_hat0))'*(y1(:,i)-W(:,:,i)'*y0(:,i) - (mu_hat1-W(:,:,i)'*mu_hat0));
            if mse < minMSE
                minMSE = mse;
                optlambda = lambda;
                Wp(:,:,ii) = W;
